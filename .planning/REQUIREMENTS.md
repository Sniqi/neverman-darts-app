...
- [x] **DISP-02**: On tablet, player can switch the app into a fullscreen display view
...
- [x] **FLOW-02**: After a configurable number of legs, a pause screen with countdown timer (e.g. 5 min) appears on both views; match continues after timer or button press
- [x] **FLOW-03**: An in-progress match survives a browser reload or crash and can be resumed exactly where it stopped
...
- [x] **STAT-07**: Player can view lifetime statistics per profile (averages, checkout %, score bands, records)
- [x] **STAT-08**: Player can view a statistics dashboard with charts (score distribution, average trend over time, darts per leg, win rate)
