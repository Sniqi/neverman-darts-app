...
stopped_at: v1.2 Restyling milestone shipped & archived (2026-07-14, verified closeout) — ROADMAP/REQUIREMENTS archived to milestones/, PROJECT.md evolved, tag v1.2 created. No active milestone; next is /gsd-new-milestone.
last_updated: "2026-07-14T20:50:01.565Z"
...
**Current focus:** v1.2 Restyling shipped & archived (2026-07-14, verified closeout). No active milestone — start the next with `/gsd-new-milestone`.
...
Cleared at v1.2 milestone close (2026-07-14). The durable log lives in PROJECT.md → Key Decisions; per-phase implementation decisions are archived in the phase SUMMARY files under `milestones/v1.2-phases/08–12`. The v1.2 platform/convention decisions that outlive the milestone — Chrome-90 Cast receiver → precomputed static `rgba()` (no `color-mix()`), self-hosted WOFF2 Barlow fonts, shared `.btn`/`.switch` primitives in `src/styles/components.css`, the no-provisional-color regression guard (`design-tokens.test.ts`), and pause-state Cast publishing via `#broadcastPause()` → `#publishToCast()` — are recorded in PROJECT.md → Key Decisions.
...
_(Resolved at v1.2 close: the pause-on-Chromecast delivery gap and the WR-01 stale-trigger-snapshot risk — pause state now publishes to Cast on every tick, watched on-device with no visible flash; debug session `debug/resolved/pause-not-shown-on-receiver.md`.)_
...
Stopped at: v1.2 Restyling milestone shipped & archived (verified closeout) — milestone artifacts archived, PROJECT.md fully evolved, retrospective updated, tag v1.2 created.
Resume file: None
