...
**Domain:** Offline-first, touch-optimized darts scoring PWA (X01 steel darts), static-hosted on GitHub Pages, single-device with a second spectator window on PC
**Researched:** 2026-06-10
**Confidence:** HIGH (core framework, build, PWA, persistence, cross-window — verified against npm + official docs); MEDIUM (dartboard rendering approach — pattern-verified, no single canonical library)
...
npm install dexie svelte-i18n

# PWA + static adapter (adapter-static usually added by scaffolder; install if not)
npm install -D vite-plugin-pwa @sveltejs/adapter-static
...
**Recommendation:** Render the board as **SVG** with one `<path>` per scoring region, driven by **Pointer Events** (`pointerdown`), and optionally back it with **polar-coordinate math** for robustness.
...
**Recommendation:** **BroadcastChannel** for live updates + **IndexedDB (Dexie)** as the source of truth.
...
**If you prioritize smallest bundle / best perf on low-end tablets:**
- Use SvelteKit + adapter-static (recommended). Smallest runtime, lightest hydration.

**If a future contributor/team strongly prefers React:**
...
**If you want zero framework runtime:**
...
---
*Stack research for: offline-first touch darts scoring PWA on GitHub Pages*
*Researched: 2026-06-10*
