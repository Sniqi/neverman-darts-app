// Lazy wrappers — the DS bundle (ds-base.js) loads async, so resolve components at render time.
const NS = () => window.NevermanDartsDesignSystem_61370c;
const wrap = (name) => (props) => { const C = NS()[name]; return <C {...props} />; };
const Button = wrap('Button'), Chip = wrap('Chip'), SegmentedControl = wrap('SegmentedControl'),
  Stepper = wrap('Stepper'), ToggleRow = wrap('ToggleRow'), ConfirmDialog = wrap('ConfirmDialog'),
  Dartboard = wrap('Dartboard'), Numpad = wrap('Numpad'), VisitStrip = wrap('VisitStrip'),
  ScoreCard = wrap('ScoreCard'), MatchHeader = wrap('MatchHeader'), PlayerPanel = wrap('PlayerPanel'),
  StatCard = wrap('StatCard');
...
function HistoryScreen({ go }) {
  const HistoryRow = wrap('HistoryRow');
  return (
...
// ── Root (router + DS-bundle readiness gate) ──────────────────────────────
function DartsAppRoot() {
  const [ready, setReady] = React.useState(!!NS());
  React.useEffect(() => {
    if (ready) return;
    const t = setInterval(() => { if (NS()) { setReady(true); clearInterval(t); } }, 50);
    return () => clearInterval(t);
  }, [ready]);
  const [route, setRoute] = React.useState(() => {
    try { return JSON.parse(localStorage.getItem('nvd-template-route')) || { screen: 'start' }; }
    catch { return { screen: 'start' }; }
  });
  const go = (screen, config) => {
    const r = { screen, config };
    setRoute(r);
    try { localStorage.setItem('nvd-template-route', JSON.stringify(r)); } catch {}
  };
  if (!ready) return null;
  const S = { start: StartScreen, setup: SetupScreen, match: MatchScreen, display: DisplayScreen, history: HistoryScreen, stats: StatsScreen }[route.screen] || StartScreen;
  return <div style={{height:'100%',position:'relative'}}><S go={go} config={route.config} /></div>;
}

Object.assign(window, { StartScreen, SetupScreen, MatchScreen, DisplayScreen, HistoryScreen, StatsScreen, DartsAppRoot });
